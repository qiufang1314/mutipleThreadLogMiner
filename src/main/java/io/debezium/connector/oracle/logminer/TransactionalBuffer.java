...
public final class TransactionalBuffer implements AutoCloseable {
    ...
    void undoDmlOperation(String transactionId, String undoRowId, TableId tableId) {
        ...
        /*处理savepoint rollback bug*/

        int deleteObjIdx = -1;
        for (int idx = transaction.events.size() - 1; idx >= 0; idx--) {
            LogMinerEvent o = transaction.events.get(idx);
        ...
        if (deleteObjIdx != -1){
            transaction.events.remove(deleteObjIdx);
            LOGGER.info("transaction.events.size = {}" ,transaction.events.size() );
        }
        ...
        try {
            ...
            for (LogMinerEvent event : transaction.events) {
                ...
                if (smallestScn == null || scn.compareTo(smallestScn) < 0) {
                    ...
                    if (printTime==0){
                        LOGGER.info("TransactionalBuffer commit set offsetContext.setSCN() to event.getScn() = {}" ,event.getScn());
                    }
                ...
                // TODO Garry: 加上时区
                offsetContext.setSourceTime(timestamp.toInstant().minusSeconds(getTotalOffsetSeconds()));
    ...
    private long getTotalOffsetSeconds() {
        Class<? extends OracleStreamingChangeEventSourceMetrics> aClass = streamingMetrics.getClass();
        try {
            Field offsetSecondsField = aClass.getDeclaredField("offsetSeconds");
            if (offsetSecondsField != null) {
                offsetSecondsField.setAccessible(true);
                AtomicInteger offsetSeconds = (AtomicInteger) offsetSecondsField.get(streamingMetrics);
    ...
    Scn updateOffsetContext(OracleOffsetContext offsetContext, EventDispatcher<TableId> dispatcher)
        ...
        if (transactions.isEmpty()) {
            if (!maxCommittedScn.isNull()) {
                long printTime = System.currentTimeMillis() % 30;
                if (printTime==0){
                    LOGGER.info("TransactionalBuffer updateOffsetContext set offsetContext.setSCN() to maxCommittedScn = {}" ,maxCommittedScn);
                }
            ...
            if (!minStartScn.isNull()) {
                ...
                Scn subtract = minStartScn.subtract(Scn.valueOf(1));
                long printTime = System.currentTimeMillis() % 30;
                if (printTime==0){
                    LOGGER.info("TransactionalBuffer updateOffsetContext set offsetContext.setSCN() to minStartScn.subtract(Scn.valueOf(1)) = {}" ,subtract);
                }

                offsetContext.setScn(subtract);
                dispatcher.dispatchHeartbeatEvent(offsetContext);
            } else {
