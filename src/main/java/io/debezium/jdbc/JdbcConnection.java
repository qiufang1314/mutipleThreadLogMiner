...
public class JdbcConnection implements AutoCloseable {
    ...
    public String buildSelectWithRowLimits(TableId tableId, int limit, String projection, Optional<String> condition, String orderBy) {
        ...
        sql
                .append(projection)
                .append(" FROM ");
        sql.append(quotedTableIdWithNoDatabase(tableId));
    ...
    // todo jace oracle 沒有limit寫法，只有rownumber，這裡不知道為什麼需要limit，先拋棄
    public String buildSelect(TableId tableId, int limit, String projection, Optional<String> condition,Optional<String> additionalCondition, String orderBy) {
        final StringBuilder sql = new StringBuilder("SELECT ");
        sql
                .append(projection)
                .append(" FROM ");
    ...
    // todo jace create empty row
    public <T extends DatabaseSchema<TableId>> Object[] emptyRowToArray(Table table, T databaseSchema, ResultSet rs,
                                                                        ColumnUtils.ColumnArray columnArray,HashMap<String,String> primaryKV)
            throws SQLException {
        ResultSetMetaData metaData = rs.getMetaData();
        final Object[] row = new Object[columnArray.getGreatestColumnPosition()];
        for (int i = 0; i < columnArray.getColumns().length; i++) {
//            columnArray
//            Set<Entry<String, String>> entries = primaryKV.entrySet();
            boolean isPrimaryKey = false;
            for (Entry<String, String> entries: primaryKV.entrySet()){
                if (metaData.getColumnName(i + 1).equalsIgnoreCase(entries.getKey()) ){
                    row[columnArray.getColumns()[i].position() - 1] = entries.getValue();
                    isPrimaryKey = true;
                }
            }
            if(!isPrimaryKey){
                row[columnArray.getColumns()[i].position() - 1] =null;
            }
        }
        return row;
    }
    ...
    public String quotedTableIdWithNoDatabase(TableId tableId) {
        return tableId.toDoubleQuotedString().split("\\.",2)[1];
    }
}
