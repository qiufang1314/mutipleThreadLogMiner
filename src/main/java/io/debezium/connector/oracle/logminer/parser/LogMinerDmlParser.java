        ...
        for (; index < sql.length(); ++index) {
            ...
            if (c == '(' && !inQuote && !inValues) {
                ...
                if (sql.charAt(start) == '\'' && sql.charAt(index - 1) == '\'') {
                    ...
                    values[position] = sql.substring(start + 1, index - 1).replace("''","'");
                }
                else {
                    // use value as-is
                    String s = sql.substring(start, index).replace("''","'");
            ...
            if (c == '"' && inColumnName) {
                ...
                if (inSingleQuote) {
                    ...
                    if (nested == 0) {
                        ...
                        newValues[position] = sql.substring(start + 1, index).replace("''","'");
                        start = index + 1;
                        inColumnValue = false;
                        inColumnName = false;
                    }
                ...
                if (c == '(') {
                    ...
                    String value = sql.substring(start, index).replace("''","'");
        ...
        for (; index < sql.length(); ++index) {
            ...
            if (c == '"' && inColumnName) {
                ...
                if (inSingleQuote) {
                    ...
                    if (nested == 0) {
                        int position = LogMinerHelper.getColumnIndexByName(currentColumnName, table);
                        values[position] = sql.substring(start + 1, index).replace("''","'");
                        start = index + 1;
                        inColumnValue = false;
                        inColumnName = false;
                    }
