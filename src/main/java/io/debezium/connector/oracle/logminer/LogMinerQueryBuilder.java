...
public class LogMinerQueryBuilder {
    ...
    public static String build(OracleConnectorConfig connectorConfig, OracleDatabaseSchema schema, String userName) {
        ...
        // todo garry, remove missing scn
        query.append("(OPERATION_CODE IN (6,7,36)");
        ...
        if (!schema.storeOnlyCapturedTables()) {
            ...
            query.append(" OR ").append(buildDdlPredicate(userName, connectorConfig)).append(" ");
...
//        query.append("AND TABLE_NAME != '").append(SqlUtils.LOGMNR_FLUSH_TABLE).append("' ");
    ...
    /**
     * 大量时间查不到数据，就直接不加条件过滤，防止start scn 不往前走
     * @param connectorConfig
     * @param schema
     * @param userName
     * @return
     */
    public static String buildFastFilterView(OracleConnectorConfig connectorConfig, OracleDatabaseSchema schema, String userName) {
        final StringBuilder query = new StringBuilder(1024);
        query.append("SELECT SCN, OPERATION_CODE, TABLE_NAME, SEG_OWNER ");
        query.append("FROM ").append(LOGMNR_CONTENTS_VIEW).append(" ");
        ...
        query.append("AND ");
        query.append(" OPERATION_CODE IN (6,7,36,1,2,3,5)");
     ...
     */
    private static String buildDdlPredicate(String userName, OracleConnectorConfig connectorConfig) {
        final StringBuilder predicate = new StringBuilder(256);
        predicate.append("(OPERATION_CODE = 5 ");
        String userNamePredicate = schemaCondition("USERNAME", connectorConfig);
        if (!Strings.isNullOrEmpty(userNamePredicate)) {
            predicate.append("AND ").append(userNamePredicate).append(" ");
        } else {
...
//        predicate.append("AND USERNAME NOT IN ('SYS','SYSTEM','").append(userName.toUpperCase()).append("') ");
        ...
        // todo garry: only monitor indicated schema and table
        String schemaPredicate = buildSchemaPredicate(connectorConfig);
        if (!Strings.isNullOrEmpty(schemaPredicate)) {
        ...
        // todo garry: only monitor indicated table alter ddl
        predicate.append(")");
    ...
    private static String buildSchemaPredicate(OracleConnectorConfig connectorConfig) {
        ...
        String schemaListStr = connectorConfig.schemaIncludeList();
        if (StringUtils.isNotBlank(schemaListStr)) {
            String[] schemaArr = StringUtils.split(schemaListStr, ",");
            String schemaCond = Arrays.stream(schemaArr).map(o -> "'" + o + "'").collect(Collectors.joining(","));
            predicate.append(" SEG_OWNER in (").append(schemaCond).append(")");
        }
        return predicate.toString();
    }

    private static String schemaCondition(String fieldName, OracleConnectorConfig connectorConfig) {
        StringBuilder predicate = new StringBuilder();
        String schemaListStr = connectorConfig.schemaIncludeList();
        if (StringUtils.isNotBlank(schemaListStr)) {
            String[] schemaArr = StringUtils.split(schemaListStr, ",");
            String schemaCond = Arrays.stream(schemaArr).map(o -> "'" + o + "'").collect(Collectors.joining(","));
            predicate.append(fieldName).append(" in (").append(schemaCond).append(")");
        }
        return predicate.toString();
    }
    ...
    private static String buildTablePredicate(OracleConnectorConfig connectorConfig) {
        ...
        return predicate.toString();*/
        StringBuilder predicate = new StringBuilder();
        String condStr = connectorConfig.tableIncludeList();
        if (StringUtils.isNotBlank(condStr)) {
            String[] condArr = StringUtils.split(condStr, ",");
            String compositedCond = Arrays.stream(condArr)
                    .map(o -> StringUtils.substringAfter(o, "."))
                    .map(o -> "'" + o + "'").collect(Collectors.joining(","));
            predicate.append(" TABLE_NAME in (").append(compositedCond).append(")");
        }
        return predicate.toString();
    }
